...
const initialMessages: ChatMessage[] = [
  { id: 'demo-user', role: 'user', content: 'What is CIS Control 1?' },
  { id: 'demo-assistant', role: 'assistant', label: 'DUMMY RESPONSE', content: 'CIS Control 1 focuses on maintaining an accurate inventory of enterprise assets. This placeholder validates the chat layout before live response streaming is added.' },
]
...
export function ChatWorkspace() {
  ...
  const [sources, setSources] = useState<Awaited<ReturnType<typeof sentinelApi.ask>>['sources']>([])
  ...
  async function submit(event?: FormEvent) {
    ...
    setQuestion(''); setLoading(true); setError(''); setSources([])
    try {
      const result = await sentinelApi.ask(value)
      setMessages((current) => [...current, { id: `assistant-${Date.now()}`, role: 'assistant', label: result.mode.toUpperCase(), content: result.answer }])
      setSources(result.sources)
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to reach the RAG backend.') }
